                    ...
                    <div class="sectionFoot-B1">
                        <div class="sectionFloat-A1 addpage_style">
                            ...
                            <div class="goToPages">
                                ...
                                echo Yii::app()->request->BaseUrl;?>/showroom/element/id/<?php echo isset($_GET['id'])?$_GET['id']:""; ?><?php 
                                echo isset($_GET['unbind']) ? "/unbind/":"";?>/page/" class="clickPageIcon" value="确定"/>&nbsp;&nbsp;
                            </div>
                        </div>
                    </div>
                </td>
...
<script type="text/javascript">
   ...
   //删除元素
    ...
    function delbatch(){
        ...
        if(data.length > 0){
            if(confirm('确定要删除选择项')){
            $.post('<?php echo CHtml::normalizeUrl(array('/showroom/deletecate')); ?>',{'id[]':data}, function (dat) {
    ...
    function changeBindType(cbt){
        var $t = $(cbt).val();
        if($t == 'unbind'){
            window.location.href = '/showroom/element/id/'+<?php echo isset($_GET['id']) ? $_GET['id'] :""; ?> +"/unbind";
        }else{
            window.location.href = '/showroom/element/id/'+<?php echo isset($_GET['id']) ? $_GET['id'] :""; ?>;
        }
    }
</script>